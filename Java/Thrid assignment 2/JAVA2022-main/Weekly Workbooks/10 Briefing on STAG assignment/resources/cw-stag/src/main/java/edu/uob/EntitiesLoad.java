...
public class EntitiesLoad {
    ...
    public void removeFurnitureName(String key, String entity){
        ArrayList<GameEntity> artefactRomve = entities.get(key).get(2);
        for(int i = 0; i < artefactRomve.size(); i++){
            if(artefactRomve.get(i).getName().equals(entity)){
    ...
    public boolean isCharacterExist(String key, String name){
        for(GameEntity entity1 : entities.get(key).get(3)){
            if(entity1.getName().equals(name)){
                return true;
            }
        }
    ...
    public boolean isPathExist(String key, String toName){
        ArrayList<String> toPlaces = paths.getPath(key);
        if(toPlaces.contains(toName)){
                return true;
        }
        return false;
    }

    public void removePath(String key, String toName){
        ArrayList<String> toPlaces = paths.getPath(key);
        if(toPlaces.contains(toName)){
            paths.removePath(key,toName);
        }
    }
    public void removeCharacterName(String nameToSearch){
        for (ArrayList<ArrayList<GameEntity>> outerList : entities.values()) {
            for (Iterator<ArrayList<GameEntity>> outerListIterator = outerList.iterator(); outerListIterator.hasNext();) {
                ArrayList<GameEntity> innerList = outerListIterator.next();
                for (Iterator<GameEntity> innerListIterator = innerList.iterator(); innerListIterator.hasNext();) {
                    GameEntity entity = innerListIterator.next();
                    if (entity.getName().equals(nameToSearch)) {
                        innerListIterator.remove(); // Remove the matching GameEntity
                    }
                }
            }
        }
